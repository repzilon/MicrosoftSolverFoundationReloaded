...
namespace Microsoft.SolverFoundation.Solvers
{
	...
	/// </summary>
	internal sealed class Greater : CspInequality
	{
		internal override string Name => ">";

		/// <summary> A boolean which is equal to whether a sequence of integers is strictly descending.
		/// </summary>
		internal Greater(ConstraintSystem solver, params CspSolverTerm[] inputs)
			: base(solver, inputs)
		{
		}
		...
		internal override CspSolverTerm Scan(out bool isFalse, out bool isTrue)
		{
			...
			int num2 = _rgLo[num - 1];
			int num3 = _rgHi[num - 1];
			isFalse = false;
			isTrue = true;
			...
			while (0 <= num4)
			{
				if (!isFalse && _rgHi[num4] <= num2)
				{
					result = _inputs[num4];
					isTrue = false;
					isFalse = true;
				}
				num2 = Math.Max(num2, _rgLo[num4]);
				if (isTrue)
				{
					isTrue = num3 < _rgLo[num4];
					num3 = _rgHi[num4];
				}
				num4--;
			}
		...
		{
			return newModel.Greater(inputs);
		}
		...
		internal override void RecomputeValue(LocalSearchSolver ls)
		{
			...
			else
			{
				...
				for (int i = 1; i < num; i++)
				{
					...
					num2 = BooleanFunction.And(num2, BooleanFunction.LessStrict(integerValue, r));
				}
			}
			ls[this] = num2;
		}
		...
		internal override void RecomputeGradients(LocalSearchSolver ls)
		{
			...
			for (int i = 1; i < _inputs.Length; i++)
			{
				...
				valueWithGradients = Gradients.And(valueWithGradients, Gradients.LessStrict(integerGradients, y));
			}
