...
namespace Microsoft.SolverFoundation.Solvers
{
	...
	/// </summary>
	internal sealed class Min : CspFunction
	{
		internal override string Name => "Min";

		/// <summary> An integer equals the minimum of a sequence of integers.
		/// </summary>
		internal Min(ConstraintSystem solver, params CspSolverTerm[] inputs)
			: base(solver, inputs)
		{
		...
		internal override bool Propagate(out CspSolverTerm conflict)
		{
			...
			for (int j = 0; j < _inputs.Length; j++)
			{
				...
				flag |= cspSolverTerm.Intersect(first, 1073741823, out conflict);
				if (conflict != null)
				{
					return flag;
				}
				int first2 = cspSolverTerm.First;
				int last3 = cspSolverTerm.Last;
				if (first2 <= num3)
				{
					num++;
					num2 = j;
		...
		internal override void RecomputeGradients(LocalSearchSolver ls)
		{
			...
			foreach (CspSolverTerm term in inputs)
			{
				...
				valueWithGradients = Gradients.Min(valueWithGradients, integerGradients);
			}
		...
		private int RecomputeMinValue(LocalSearchSolver ls)
		{
			int num = CspSolverTerm.MaxSafePositiveValue;
			CspSolverTerm[] inputs = _inputs;
