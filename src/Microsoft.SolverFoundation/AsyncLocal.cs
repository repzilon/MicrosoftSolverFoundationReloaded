...
// https://github.com/dotnet/runtime/blob/1d1bf92fcf43aa6981804dc53c5174445069c9e4/src/libraries/System.Private.CoreLib/src/System/Threading/AsyncLocal.cs
// Original file was licensed under the MIT license but is incorporated inside
// a Ms-PL licensed project. Is this version covered by the MIT license or Ms-PL?
...
namespace System.Threading
{
	...
	{
		private const BindingFlags kInternalStaticMethod = BindingFlags.InvokeMethod | BindingFlags.NonPublic | BindingFlags.Static;
		private static readonly MethodInfo getLocalValueMethod = typeof(ExecutionContext).GetMethod(nameof(GetLocalValue), kInternalStaticMethod);
		private static readonly MethodInfo setLocalValueMethod = typeof(ExecutionContext).GetMethod(nameof(SetLocalValue), kInternalStaticMethod);
		...
		{
			get {
				object value = GetLocalValue(this);
				return typeof(T).IsValueType && value == null ? default : (T)value;
			}
			set {
				SetLocalValue(this, value, _valueChangedHandler != null);
			}
		}

		private static object GetLocalValue(IAsyncLocal local)
		{
			return getLocalValueMethod.Invoke(null, new object[] { local });
		}

		private static void SetLocalValue(IAsyncLocal local, object newValue, bool needChangeNotifications)
		{
			setLocalValueMethod.Invoke(null, new object[] { local, newValue, needChangeNotifications });
		}

		void IAsyncLocal.OnValueChanged(object previousValueObj, object currentValueObj, bool contextChanged)
		{
			Debug.Assert(_valueChangedHandler != null);
	...
	internal static class AsyncLocalValueMap
	{
		...
		public static IAsyncLocalValueMap Create(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
		{
			...
			return value != null || !treatNullValueAsNonexistent ?
				new OneElementAsyncLocalValueMap(new KeyValuePair<IAsyncLocal, object>(key, value)) :
				Empty;
		}
		...
		private sealed class EmptyAsyncLocalValueMap : IAsyncLocalValueMap
		{
			public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
			{
				...
				if (value != null || !treatNullValueAsNonexistent) {
					return new OneElementAsyncLocalValueMap(new KeyValuePair<IAsyncLocal, object>(key, value));
				} else {
					return this;
				}
			}
		...
		private sealed class OneElementAsyncLocalValueMap : IAsyncLocalValueMap
		{
			...
			public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
			{
				if (value != null || !treatNullValueAsNonexistent) {
					...
					KeyValuePair<IAsyncLocal, object> newItem = new KeyValuePair<IAsyncLocal, object>(key, value);
					if (ReferenceEquals(key, _item0.Key)) {
						return new OneElementAsyncLocalValueMap(newItem);
					} else {
						return new TwoElementAsyncLocalValueMap(_item0, newItem);
					}
				} else {
		...
		private sealed class TwoElementAsyncLocalValueMap : IAsyncLocalValueMap
		{
			...
			public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
			{
				if (value != null || !treatNullValueAsNonexistent) {
					...
					KeyValuePair<IAsyncLocal, object> newItem = new KeyValuePair<IAsyncLocal, object>(key, value);
					if (ReferenceEquals(key, _item0.Key)) {
						return new TwoElementAsyncLocalValueMap(newItem, _item1);
					} else if (ReferenceEquals(key, _item1.Key)) {
						return new TwoElementAsyncLocalValueMap(_item0, newItem);
					} else {
						return new ThreeElementAsyncLocalValueMap(_item0, _item1, newItem);
					}
				} else {
		...
		private sealed class ThreeElementAsyncLocalValueMap : IAsyncLocalValueMap
		{
			...
			public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
			{
				if (value != null || !treatNullValueAsNonexistent) {
					...
					KeyValuePair<IAsyncLocal, object> newItem = new KeyValuePair<IAsyncLocal, object>(key, value);
					if (ReferenceEquals(key, _item0.Key)) {
						return new ThreeElementAsyncLocalValueMap(newItem, _item1, _item2);
					} else if (ReferenceEquals(key, _item1.Key)) {
						return new ThreeElementAsyncLocalValueMap(_item0, newItem, _item2);
					} else if (ReferenceEquals(key, _item2.Key)) {
						return new ThreeElementAsyncLocalValueMap(_item0, _item1, newItem);
					} else {
						return new FourElementAsyncLocalValueMap(_item0, _item1, _item2, newItem);
					}
				} else {
					// If the key exists in this map, remove it by downgrading to a one-element map without the key.  Otherwise,
					// there's nothing to add or remove, so just return this map.
		...
		private sealed class FourElementAsyncLocalValueMap : IAsyncLocalValueMap
		{
			...
			public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
			{
				if (value != null || !treatNullValueAsNonexistent) {
					...
					KeyValuePair<IAsyncLocal, object> newItem = new KeyValuePair<IAsyncLocal, object>(key, value);
					if (ReferenceEquals(key, _item0.Key)) {
						return new FourElementAsyncLocalValueMap(newItem, _item1, _item2, _item3);
					} else if (ReferenceEquals(key, _item1.Key)) {
						return new FourElementAsyncLocalValueMap(_item0, newItem, _item2, _item3);
					} else if (ReferenceEquals(key, _item2.Key)) {
						return new FourElementAsyncLocalValueMap(_item0, newItem, _item2, _item3);
					} else if (ReferenceEquals(key, _item3.Key)) {
						return new FourElementAsyncLocalValueMap(_item0, _item1, _item2, newItem);
					} else {
						return new MultiElementAsyncLocalValueMap(_item0, _item1, _item2, _item3, newItem); // upgrade to a multi
					}
				} else {
		...
		private sealed class MultiElementAsyncLocalValueMap : IAsyncLocalValueMap
		{
			...
			internal MultiElementAsyncLocalValueMap(params KeyValuePair<IAsyncLocal, object>[] keyValues)
			{
				Debug.Assert(keyValues.Length >= 5 && keyValues.Length <= MaxMultiElements);
			...
			public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
			{
				...
				for (int i = 0; i < _keyValues.Length; i++) {
					if (ReferenceEquals(key, _keyValues[i].Key)) {
						...
						if (value != null || !treatNullValueAsNonexistent) {
							// Create a new map of the same size that has all the same pairs, with this new key/value pair overwriting the old.
							KeyValuePair<IAsyncLocal, object>[] newValues = _keyValues.ToArray();
							newValues[i] = new KeyValuePair<IAsyncLocal, object>(key, value);
							...
							if (i == 0) {
								return new FourElementAsyncLocalValueMap(_keyValues[1], _keyValues[2], _keyValues[3], _keyValues[4]);
							} else if (i == 1) {
								return new FourElementAsyncLocalValueMap(_keyValues[0], _keyValues[2], _keyValues[3], _keyValues[4]);
							} else if (i == 2) {
								return new FourElementAsyncLocalValueMap(_keyValues[0], _keyValues[1], _keyValues[3], _keyValues[4]);
							} else if (i == 3) {
								return new FourElementAsyncLocalValueMap(_keyValues[0], _keyValues[1], _keyValues[2], _keyValues[4]);
							} else {
								return new FourElementAsyncLocalValueMap(_keyValues[0], _keyValues[1], _keyValues[2], _keyValues[3]);
							}
						} else {
							// We have enough elements remaining to warrant a multimap.  Create a new one and copy all the
				...
				// If with the addition we can still fit in a multimap, create one.
				if (_keyValues.Length < MaxMultiElements) {
					var newValues = new KeyValuePair<IAsyncLocal, object>[_keyValues.Length + 1];
					Array.Copy(_keyValues, newValues, _keyValues.Length);
					newValues[_keyValues.Length] = new KeyValuePair<IAsyncLocal, object>(key, value);
					return new MultiElementAsyncLocalValueMap(newValues);
				}
		...
		private sealed class ManyElementAsyncLocalValueMap : Dictionary<IAsyncLocal, object>, IAsyncLocalValueMap
		{
			...
			public IAsyncLocalValueMap Set(IAsyncLocal key, object value, bool treatNullValueAsNonexistent)
			{
				...
				// If the value being set exists, create a new many map, copy all the elements from this one,
				...
				if (containsKey) {
					// If the new count would be within range of a multimap instead of a many map,
					// downgrade to the multimap, which uses less memory and is faster to access.
