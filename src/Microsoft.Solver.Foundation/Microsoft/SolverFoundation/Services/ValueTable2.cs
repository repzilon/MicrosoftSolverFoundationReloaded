...
{
	internal sealed class ValueTable2<TValue> : ValueTable<TValue>
	{
		private struct Key : IEquatable<Key>
		{
			...
			{
				return Statics.CombineHash(key0.GetHashCode(), key1.GetHashCode());
			}

			public bool Equals(Key other)
			{
				if (key0.Equals(other.key0))
				{
		...
		public override IEnumerable<object[]> Keys
		{
			get
			{
				...
				{
					yield return new object[2] { k.key0, k.key1 };
				}
			}
		}
		...
		internal ValueTable2(Domain domain, ValueSet[] indexSets)
