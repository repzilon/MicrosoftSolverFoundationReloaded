...
namespace Microsoft.SolverFoundation.Solvers
{
	...
	/// </summary>
	internal sealed class EvaluableLessStrict : EvaluableBooleanTerm
	{
		private readonly EvaluableNumericalTerm[] Inputs;

		internal override TermModelOperation Operation => TermModelOperation.Less;

		internal EvaluableLessStrict(EvaluableNumericalTerm[] inputs)
		...
		internal override void Recompute(out bool change)
		{
			...
			{
				num = EvaluableBinaryAnd.Apply(num, EvaluableBinaryLessStrict.Apply(Inputs[i - 1].Value, Inputs[i].Value));
			}
		...
		public override EvaluableTerm Substitute(Dictionary<EvaluableTerm, EvaluableTerm> map)
		{
			...
			if (flag)
			{
				...
				return new EvaluableLessStrict(array);
			}
			return null;
		}
	}
